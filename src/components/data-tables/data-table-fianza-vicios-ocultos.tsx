  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const deleteFianza = useMutation(async () => {
        ...
        console.log(resData);
        queryClient.invalidateQueries(["fianzaViciosOcultos"]);
      });
      return (
        <DropdownMenu>
...
export function DataTableFianzaViciosOcultos({
  contratoId,
}: {
  contratoId: string;
}) {
  const fianzaViciosOcultosQuery = useQuery({
    queryKey: ["fianzaViciosOcultos", contratoId],
    queryFn: async () => {
      const token = localStorage.getItem("token");
      console.log("Token:", token);
      const res = await fetch(
        `http://localhost:3001/fianza/contrato/${contratoId}/fianza-oculto`,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const fianzaViciosOcultosParse = Fianza.array().safeParse(resData);
      if (!fianzaViciosOcultosParse.success) {
        console.error(fianzaViciosOcultosParse.error);
        throw new Error(fianzaViciosOcultosParse.error.toString());
      }
      return fianzaViciosOcultosParse.data;
    },
  });
  ...
  const table = useReactTable({
    data: fianzaViciosOcultosQuery.data || [],
    columns,
  ...
  const exportToExcel = (fianzaViciosOcultos: Fianza[]) => {
    if (!fianzaViciosOcultos || fianzaViciosOcultos.length === 0) {
      alert("No hay datos disponibles para exportar.");
      return;
    }

    const datos = fianzaViciosOcultos.map((fianzaViciosOcultos) => ({
      ID: fianzaViciosOcultos.idfianza ?? "N/A",
      TIPO: fianzaViciosOcultos.tipo ?? "N/A",
      TIPO_CAMBIO: fianzaViciosOcultos.tipodecambio ?? "N/A",
      INICIO: fianzaViciosOcultos.inicio ?? "N/A",
      FIN: fianzaViciosOcultos.fin ?? "N/A",
      POLIZA: fianzaViciosOcultos.poliza ?? "N/A",
      ASEGURADORA: fianzaViciosOcultos.aseguradora ?? "N/A",
      MONTO: fianzaViciosOcultos.monto ?? "N/A",
    }));
  ...
  // Exportar a pdf
  const exportToPDF = (fianzaViciosOcultos: Fianza[]) => {
    if (!fianzaViciosOcultos || fianzaViciosOcultos.length === 0) {
      alert("No hay datos disponibles para exportar.");
      return;
    }
    ...
    const generarDatosBloque = (
      fianzaViciosOcultos: Fianza[],
      campos: { header: string; key: string }[]
    ) => {
      return fianzaViciosOcultos.map((fianzaViciosOcultos) => {
        const bloque: Record<string, string> = {};
        campos.forEach(({ header, key }) => {
          const valor =
            key
              .split(".")
              .reduce((acc, curr) => acc?.[curr], fianzaViciosOcultos as any) ||
            "N/A";
          bloque[header] = valor;
        });
        return bloque;
    ...
    const datosBloque1 = generarDatosBloque(fianzaViciosOcultos, camposBloque1);
    ...
    const text1 = "DATOS DE FIANZA DE ViciosOcultos";
    ...
    doc.save("Fianza_ViciosOcultos_oceanus.pdf");
  };

  // Imprimir datos
  const handlePrint = (fianzaViciosOcultos: Fianza[]) => {
    if (!fianzaViciosOcultos || fianzaViciosOcultos.length === 0) {
    ...
    // Primer título: "DATOS DE PERSONALES GENERALES" (centrado en el centro de la página)
    const text1 = "DATOS DE FIANZA ViciosOcultos";
    const fontSize1 = doc.getFontSize(); // Obtener el tamaño de la fuente en uso
    const textWidth1 =
      (doc.getStringUnitWidth(text1) * fontSize1) / doc.internal.scaleFactor;
    const xPosition1 = (pageWidth - textWidth1) / 2; // Centrado horizontalmente
      ...
      <div className="flex items-center py-4">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            ...
            <DropdownMenuItem
              onClick={() => exportToExcel(fianzaViciosOcultosQuery.data || [])}
            >
              Exportar a Excel
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => exportToPDF(fianzaViciosOcultosQuery.data || [])}
            >
              Exportar a PDF
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => handlePrint(fianzaViciosOcultosQuery.data || [])}
            >
              Imprimir
            </DropdownMenuItem>
          </DropdownMenuContent>
