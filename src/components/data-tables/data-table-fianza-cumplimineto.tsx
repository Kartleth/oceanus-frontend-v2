  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const deleteFianza = useMutation(async () => {
        ...
        console.log(resData);
        queryClient.invalidateQueries(["fianzaCumplimiento"]);
      });
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            >
              <Link
                to={`/contratos/${contratoId}/fianza-Cumplimiento/detalles/${row.original.idfianza}`}
              >
                Ver detalles
              </Link>
            </DropdownMenuItem>
            ...
            <DropdownMenuItem asChild>
              <Link to={`/contratos/${contratoId}/fianza-cumplimiento/editar-fianza-cumplimiento/${row.original.idfianza}`}>Editar</Link>
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => {
                deleteFianza.mutate();
...
export function DataTableFianzaCumplimiento({
  contratoId,
}: {
  contratoId: string;
}) {
  const fianzaCumplimientoQuery = useQuery({
    queryKey: ["fianzaCumplimiento", contratoId],
    queryFn: async () => {
      const token = localStorage.getItem("token");
      console.log("Token:", token);
      const res = await fetch(
        `http://localhost:3001/fianza/contrato/${contratoId}/fianza-cumplimiento`,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const fianzaCumplimientoParse = Fianza.array().safeParse(resData);
      if (!fianzaCumplimientoParse.success) {
        console.error(fianzaCumplimientoParse.error);
        throw new Error(fianzaCumplimientoParse.error.toString());
      }
      return fianzaCumplimientoParse.data;
    },
  });
  ...
  const table = useReactTable({
    data: fianzaCumplimientoQuery.data || [],
    columns,
  ...
  const exportToExcel = (fianzaCumplimiento: Fianza[]) => {
    if (!fianzaCumplimiento || fianzaCumplimiento.length === 0) {
      alert("No hay datos disponibles para exportar.");
      return;
    }

    const datos = fianzaCumplimiento.map((fianzaCumplimiento) => ({
      ID: fianzaCumplimiento.idfianza ?? "N/A",
      TIPO: fianzaCumplimiento.tipo ?? "N/A",
      TIPO_CAMBIO: fianzaCumplimiento.tipodecambio ?? "N/A",
      INICIO: fianzaCumplimiento.inicio ?? "N/A",
      FIN: fianzaCumplimiento.fin ?? "N/A",
      POLIZA: fianzaCumplimiento.poliza ?? "N/A",
      ASEGURADORA: fianzaCumplimiento.aseguradora ?? "N/A",
      MONTO: fianzaCumplimiento.monto ?? "N/A",
    }));
    ...
    XLSX.writeFile(workbook, `Fianza_Cumplimiento_contrato_${contratoId}.xlsx`);
  };

  // Exportar a pdf
  const exportToPDF = (fianzaCumplimiento: Fianza[]) => {
    if (!fianzaCumplimiento || fianzaCumplimiento.length === 0) {
      alert("No hay datos disponibles para exportar.");
      return;
    }
    ...
    const generarDatosBloque = (
      ...
      return fianzaCumplimiento.map((fianzaCumplimiento) => {
        const bloque: Record<string, string> = {};
        campos.forEach(({ header, key }) => {
          const valor =
            key
              .split(".")
              .reduce((acc, curr) => acc?.[curr], fianzaCumplimiento as any) ||
            "N/A";
          bloque[header] = valor;
        });
        return bloque;
    ...
    const datosBloque1 = generarDatosBloque(fianzaCumplimiento, camposBloque1);
    ...
    const text1 = "DATOS DE FIANZA DE Cumplimiento";
    ...
    doc.save("Fianza_Cumplimiento_oceanus.pdf");
  };

  // Imprimir datos
  const handlePrint = (fianzaCumplimiento: Fianza[]) => {
    if (!fianzaCumplimiento || fianzaCumplimiento.length === 0) {
    ...
    // Primer título: "DATOS DE PERSONALES GENERALES" (centrado en el centro de la página)
    const text1 = "DATOS DE FIANZA Cumplimiento";
    const fontSize1 = doc.getFontSize(); // Obtener el tamaño de la fuente en uso
    const textWidth1 =
      (doc.getStringUnitWidth(text1) * fontSize1) / doc.internal.scaleFactor;
    const xPosition1 = (pageWidth - textWidth1) / 2; // Centrado horizontalmente
      ...
      <div className="flex items-center py-4">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            ...
            <DropdownMenuItem
              onClick={() => exportToExcel(fianzaCumplimientoQuery.data || [])}
            >
              Exportar a Excel
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => exportToPDF(fianzaCumplimientoQuery.data || [])}
            >
              Exportar a PDF
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => handlePrint(fianzaCumplimientoQuery.data || [])}
            >
              Imprimir
            </DropdownMenuItem>
          </DropdownMenuContent>
